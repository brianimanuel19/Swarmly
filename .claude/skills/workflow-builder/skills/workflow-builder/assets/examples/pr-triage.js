...
export const meta = {
  name: 'pr-triage',
  description:
    'Triage open PRs for bugs before merge, with adversarial verification of high-severity findings',
...
const reviews = await parallel(
  PRS.map(
    (pr, i) => () =>
      agent(`Review PR ${pr} for likely bugs. Inspect the diff and report concrete findings.`, {
        label: `review:${pr}`,
        phase: 'Review',
        model: 'haiku',
...
log(
  `Reviewed ${reviews.filter(Boolean).length} PRs; ${highSeverity.length} high-severity findings to verify.`,
);
